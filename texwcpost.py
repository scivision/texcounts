...
imgStem = texStem + '.png' #for paramiko
imgFN = texPath + imgStem
...
if texChanged:
   ...
   print('modification time same as before, not appending')
...
if doUpload:
    ...
    sftp = ssh.open_sftp()
    sftp.put(imgFN, serverDir+imgStem,confirm=True) #note that destination filename MUST be included!
    sftp.close()
    ssh.close()
